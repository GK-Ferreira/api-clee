...
router.get("/view", checkToken, async (req, res) => {
  await Equips.find({ D_E_L_E_T: "" })
    ...
    .catch((err) => {
      res
        .status(404)
        .json({ msg: "Não foi encontrado nenhum registro no banco" });
    });
});
...
router.post("/idEquip:id", checkToken, async (req, res) => {
  req;
});
    ...
    await Equips.findOne({
      ...
      .then((exists) => {
        if (exists) {
          ...
          Equips.findOne({ _id: req.params.id })
            .then(async (equipss) => {
              if (
                equipss.nome != nome.toUpperCase() ||
                equipss.qnt_estoque != qnt_estoque
              ) {
                ...
                await Equips.findByIdAndUpdate(filter, update, { new: true })
                  .then(() => {
                    res
                      .status(200)
                      .json({ msg: "Equipamento atualizado com sucesso!" });
                  })
                  .catch((err) => {
                    res
                      .status(404)
                      .json({ msg: "Error ao atualizar equipamento!" });
                  });
...
router.put("/delete/:id", checkToken, async (req, res) => {
  await Equips.findOne({ _id: req.params.id })
    .then(async () => {
      const filter = { _id: req.params.id };
      ...
      await Equips.findByIdAndUpdate(filter, update, { new: true })
        ...
        .catch((err) => {
          res
            .status(404)
            .json({ msg: "Error ao deletar equipamento. ERROR: " + err });
        });
    })
    .catch((err) => {
