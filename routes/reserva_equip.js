...
router.post("/searchbycod", checkToken, async (req, res) => {
  ...
  if (cod_reserva) {
    R_Equip.find({ cod_reserva: cod_reserva, D_E_L_E_T: "" })
      .lean()
      .then((r_equips) => {
        if (r_equips) {
...
router.post("/searchbyequip", checkToken, async (req, res) => {
  ...
  if (cod_equip) {
    R_Equip.find({ cod_equip: cod_equip, D_E_L_E_T: "" })
      .lean()
      .then((r_equips) => {
        if (r_equips) {
          res.status(200).json(r_equips);
...
router.post("/searchbydater", checkToken, async (req, res) => {
  ...
  if (date_reserv) {
    R_Equip.find({ date_reserv: date_reserv, D_E_L_E_T: "" })
      .lean()
      .then((r_equips) => {
        if (r_equips) {
          res.status(200).json(r_equips);
...
router.post("/searchbydatee", checkToken, async (req, res) => {
  ...
  if (date_entrega) {
    R_Equip.find({ date_entrega: date_entrega, D_E_L_E_T: "" })
      .lean()
      .then((r_equips) => {
        if (r_equips) {
          res.status(200).json(r_equips);
...
router.post("/searchbyhourr", checkToken, async (req, res) => {
  ...
  if (hora_reserva) {
    R_Equip.find({ hora_reserva: hora_reserva, D_E_L_E_T: "" })
      .lean()
      .then((r_equips) => {
        if (r_equips) {
          res.status(200).json(r_equips);
...
router.post("/searchbyhoure", checkToken, async (req, res) => {
  ...
  if (hora_entrega) {
    R_Equip.find({ hora_entrega: hora_entrega, D_E_L_E_T: "" })
      .lean()
      .then((r_equips) => {
        if (r_equips) {
          res.status(200).json(r_equips);
...
router.post("/create", checkToken, async (req, res) => {
  ...
  const date_rv = new Date(
    `${dat_rv[1]}/${dat_rv[0]}/${dat_rv[2]} ${hora_reserva}:00`
  );
  const date_ev = new Date(
    `${dat_re[1]}/${dat_re[0]}/${dat_re[2]} ${hora_entrega}:00`
  );
  ...
  if (hora_rv > hora_ev) {
    res
      .status(422)
      .json({
        msg: "HORA DA RESERVA NÃO PODE SER MAIOR QUE A HORA DA ENTREGA",
      });
  }
  ...
  if (date_rv > date_ev) {
    res
      .status(422)
      .json({
        msg: "DATA DA RESERVA NÃO PODE SER MAIOR QUE A DATA DA ENTREGA",
      });
  } else {
    ...
    R_Equip.find({
      cod_equip: cod_equip,
      $or: [{ date_reserv: date_reserv }, { date_reserv: date_entrega }],
      $or: [{ date_entrega: date_reserv }, { date_entrega: date_entrega }],
      ...
      .then((exists) => {
        ...
        //Verificando se o equipamento digitado existe ou não no sistema.
        Equips.findOne({ codigo: cod_equip, D_E_L_E_T: "" })
          .lean()
          .then((equips) => {
            if (equips) {
              soma = +qnt_equip + +sum;
              ...
              if (qnt_equip == 0 || qnt_equip == null) {
                res
                  .status(422)
                  .json({ msg: "A quantidade pedida não pode ser nula." });
              }
              if (equips.qnt_estoque == 0 || equips.status == "I") {
                res
                  .status(400)
                  .json({
                    msg: "O equipamento se encontra inativo (fora de estoque).",
                  });
              }
              ...
              if (equips.qnt_estoque < qnt_equip) {
                res
                  .status(401)
                  .json({
                    msg: "A quantidade de pedida é maior que a quantidade em estoque.",
                  });
              }
              ...
              if (equips.qnt_estoque < soma) {
                res
                  .status(400)
                  .json({
                    msg: `Equipamento não pode mais ser reservado, escolha outro horário`,
                  });
              } else {
                R_Equip.findOne()
                  .sort({ _id: -1 })
                  ...
                  .then((u_R_Equips) => {
                    u_R_Equips
                      ? (cod_reserva = u_R_Equips.cod_reserva)
                      : (cod_reserva = 0);
                    const newR_Equip = {
              ...
              res
                .status(404)
                .json({
                  msg: `Equipamento não encontrado ou fora de estoque.`,
                });
            }
          });
      })
...
router.put("/update/:id", checkToken, async (req, res) => {
  ...
  if (hora_rv > hora_ev) {
    res
      .status(422)
      .json({
        msg: "HORA DA RESERVA NÃO PODE SER MAIOR QUE A DATA DA ENTREGA",
      });
  }
          ...
          .then((equips) => {
            if (equips) {
              ...
              if (equips.qnt_estoque < soma) {
                ...
                R_Equip.findOne({ _id: req.params.id })
                  .then(async (R_Equips) => {
                    if (
                      ...
                      await R_Equip.findByIdAndUpdate(filter, update, {
                        new: true,
                      })
                        .then(() => {
                          res
                            .status(200)
                            .json({
                              msg: `Reserva de Equipamento atualizada com sucesso!`,
                            });
                        })
                        .catch((err) => {
                          res
                            .status(400)
                            .json({ msg: "Erro ao atualizar reserva!" });
                        });
                    } else {
                      // Nenhum dado foi alterado, redirecione com uma mensagem
                      res
                        .status(404)
                        .json({ msg: "Nenhuma alteração feita nos dados!" });
                    }
                  })
                  .catch((err) => {
                    res
                      .status(404)
                      .json({
                        msg: "Error ao atualizar os dados. ERROR: " + err,
                      });
                  });
              }
            } else {
...
router.put("/delete/:id", checkToken, async (req, res) => {
  await R_Equip.findOne({ _id: req.params.id })
    .then(async () => {
      const filter = { _id: req.params.id };
      ...
      await R_Equip.findByIdAndUpdate(filter, update, { new: true })
        .then(() => {
          res
            .status(200)
            .json({ msg: "Reserva de Equipamento deletada com sucesso!" });
        })
        .catch((err) => {
          res
            .status(404)
            .json({
              msg: "Error ao deletar reserva (equipamento). ERROR: " + err,
            });
        });
    })
    .catch((err) => {
