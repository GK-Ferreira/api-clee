...
router.post("/searchbycod", checkToken, async (req, res) => {
  ...
  if (cod_reserva) {
    R_Salas.find({ cod_reserva: cod_reserva, D_E_L_E_T: "" })
      .lean()
      .then((r_salass) => {
        if (r_salass) {
...
router.post("/searchbyclass", checkToken, async (req, res) => {
  ...
  if (cod_sala) {
    R_Salas.find({ cod_sala: cod_sala, D_E_L_E_T: "" })
      .lean()
      .then((r_salass) => {
        if (r_salass) {
          res.status(200).json(r_salass);
...
router.post("/searchbydater", checkToken, async (req, res) => {
  ...
  if (date_reserv) {
    R_Salas.find({ date_reserv: date_reserv, D_E_L_E_T: "" })
      .lean()
      .then((r_salass) => {
        if (r_salass) {
          res.status(200).json(r_salass);
...
router.post("/searchbydatee", checkToken, async (req, res) => {
  ...
  if (date_entrega) {
    R_Salas.find({ date_entrega: date_entrega, D_E_L_E_T: "" })
      .lean()
      .then((r_salass) => {
        if (r_salass) {
          res.status(200).json(r_salass);
...
router.post("/searchbyhourr", checkToken, async (req, res) => {
  ...
  if (hora_reserva) {
    R_Salas.find({ hora_reserva: hora_reserva, D_E_L_E_T: "" })
      .lean()
      .then((r_salass) => {
        if (r_salass) {
          res.status(200).json(r_salass);
...
router.post("/searchbyhoure", checkToken, async (req, res) => {
  ...
  if (hora_entrega) {
    R_Salas.find({ hora_entrega: hora_entrega, D_E_L_E_T: "" })
      .lean()
      .then((r_salass) => {
        if (r_salass) {
          res.status(200).json(r_salass);
    ...
    R_Salas.findOne({
      ...
      .then((exists) => {
        if (exists) {
          ...
          Salas.findOne({ codigo: cod_sala, D_E_L_E_T: "" })
            .lean()
            .then((salas) => {
              if (salas) {
                if (salas.status == "I") {
                  ...
                  R_Salas.findOne()
                    ...
                    .then((u_r_salass) => {
                      u_r_salass
                        ? (cod_reserva = u_r_salass.cod_reserva)
                        : (cod_reserva = 0);
                      const newR_Salas = {
            ...
            .then((salas) => {
              if (salas) {
                if (salas.status == "I") {
                  ...
                  R_Salas.findOne({ _id: req.params.id })
                    .then(async (r_salass) => {
                      if (
                        ...
                        await R_Salas.findByIdAndUpdate(filter, update, {
                          new: true,
                        })
                          .then(() => {
                            res.status(200).json({
                              msg: "Reserva de Sala atualizada com sucesso!",
                            });
                          })
                          .catch((err) => {
                            res
...
router.put("/delete/:id", checkToken, async (req, res) => {
  await R_Salas.findOne({ _id: req.params.id })
    .then(async () => {
      const filter = { _id: req.params.id };
      ...
      await R_Salas.findByIdAndUpdate(filter, update, { new: true })
        .then(() => {
          res
            .status(200)
            .json({ msg: "Reserva de Sala deletada com sucesso!" });
        })
        .catch((err) => {
          res
            .status(404)
            .json({ msg: "Error ao deletar reserva (sala). ERROR: " + err });
        });
    })
    .catch((err) => {
      res.status(404).json({ msg: "Sala não encontrada." });
